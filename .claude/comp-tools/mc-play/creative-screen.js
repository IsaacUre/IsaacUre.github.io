...
(async () => {
  ...
  try {
    ...
    await g.ev(() => { const b = document.querySelector('.mc-ctab[data-ct="5"]'); const r = b.getBoundingClientRect(); document.querySelector('.mc-panelwrap').dispatchEvent(new MouseEvent('mousemove', { bubbles: true, clientX: r.left + 5, clientY: r.top + 5 })); b.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, clientX: r.left + 5, clientY: r.top + 5 })); });
    ok((await g.ev(() => { const t = document.querySelector('.mc-ptip'); return t.style.display !== 'none' ? t.textContent : null; })) === 'Tools & Utilities', 'hovering a tab names it');
    await g.ev(() => { const b = document.querySelector('.mc-ctab[data-ct="5"]'); const r = b.getBoundingClientRect(); b.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, button: 2, clientX: r.left + 5, clientY: r.top + 5 })); b.dispatchEvent(new MouseEvent('contextmenu', { bubbles: true, cancelable: true, button: 2 })); window.dispatchEvent(new MouseEvent('mouseup', { bubbles: true, button: 2 })); });
    await g.sleep(60);
